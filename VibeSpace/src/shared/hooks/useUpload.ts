...
export const useUpload = () => {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    const signatureRes = await fetch(`https://vibein-os9n.onrender.com/storage/signed-url?count=${acceptedFiles.length}`);
  ...
  const uploadFiles = async (filesToUpload: UploadFile[]) => {
    ...
    await Promise.all(
      filesToUpload.map(async (file) => {
        ...
        const res = await fetch(`https://api.cloudinary.com/v1_1/dnyr37sgw/auto/upload`, {
          method: 'POST',
          body: formData,
        })
        ...
        if (res.ok) {
          ...
          setFiles((prev) =>
            prev.map((f) =>
              f.id === file.id ? { ...f, status: 'done', progress: 100, url: resJson.secure_url } : f
            )
          )
        } else {
          setFiles((prev) =>
