...
export interface RabbitmqConfig extends Options.Connect {
   protocol: string;
   hostname: string;
   port: number;
   username: string;
...
export type ConnectionOptions = Options & { recovery: RecoveryOptions | true };
...
export interface consumerSideMessageHearders extends MessageHeaders, MessagePropertyHeaders {}
...
export type requiredOptions = Required<
   Pick<Options.Publish, 'userId' | 'replyTo' | 'type' | 'contentType' | 'timestamp'>
>;
export type optionalOptions = Partial<
   Omit<Options.Publish, 'userId' | 'replyTo' | 'type' | 'contentType' | 'timestamp'>
>;
