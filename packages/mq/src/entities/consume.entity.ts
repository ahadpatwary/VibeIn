...
import { BackoffStrategy, DEFAULT_RETRY_CONFIG } from './backoffStrategy.entity.js';
import { MessageSerializer } from '../../utils/mq.serializer.js';
import { ChannelManager } from './channel.entity.js';
import {
   ExchangeConfig,
   QueueConfig,
   RetryConfig,
...
import { RabbitMqLogger, Logger } from '../../utils/mq.logger.js';
...
export abstract class BaseConsumer<T = unknown> {
    ...
    */
   protected async onDeadLetter(message: MessageEnvelope<T>, finalError: Error): Promise<void> {
   ...
   async start(): Promise<void> {
      ...
      const consumeReply = await ch.consume(
   ...
   async stop(timeoutMs = 30_000): Promise<void> {
      ...
      if (this.consumerTag) {
         const ch = await this.channelManager
            .getChannel(`consumer:${this.opts.queue.name}`)
            .catch(() => null);
         if (ch) {
