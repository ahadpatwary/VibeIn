...
export class BaseRpcClient {
    ...
    async initialize(): Promise<void> {
        this.ch = await this.channelManager.getChannel(
            `rpc:${this.requestExchange}`,
        );

        // Exclusive, auto-delete reply queue — unique per client instance
        const { queue } = await this.ch.assertQueue('', {
            exclusive: true,
            autoDelete: true,
        });
    ...
    async call<Req, Res>(
        ...
        if (!this.ch || !this.replyQueue) {
            throw new Error(
                'RpcClient not initialised — call initialize() first',
            );
        }
        ...
        return new Promise<Res>((resolve, reject) => {
            const timeout = setTimeout(() => {
                ...
                reject(
                    new Error(
                        `RPC timeout after ${timeoutMs}ms (correlationId=${correlationId})`,
                    ),
                );
            }, timeoutMs);
    ...
    private handleReply(msg: amqp.ConsumeMessage): void {
        ...
        try {
            ...
            pending.reject(
                new Error(
                    `RPC: failed to parse reply: ${(err as Error).message}`,
                ),
            );
            return;
        }
