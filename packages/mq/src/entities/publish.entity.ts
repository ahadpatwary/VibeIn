...
import { ExchangeConfig, PublishOptions, MessageEnvelope } from '../types/mq.types.js';
...
export abstract class BasePublisher {
   ...
   async initialize(): Promise<void> {
      ...
      this.logger.info('Publisher initialised', {
   ...
   async publish<T>(
      ...
      const { buffer, envelope } = MessageSerializer.serialize(
         data,
         this.exchange.name,
         routingKey,
         {
   ...
   private async publishWithConfirm(
      ...
      return new Promise<void>((resolve, reject) => {
         const ok = ch.publish(exchange, routingKey, content, opts, (err) => {
