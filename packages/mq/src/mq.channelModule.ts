...
export class ChannelRecoveryModule {
   ...
   async #createConfirmChannel(options: channelOptions): Promise<ConfirmChannel> {
      try {
         ...
         throw new RabbitMQChannelCreationException(options.name, error as Error, {});
      }
   }

   #registerEventHandlers(channel: amqp.ConfirmChannel, channelName: string): void {
