...
export const QueueBindingSchema = z.object({
    ...
    routingKey: z.string().min(1, 'Routing key required'),
    bindingArgs: z.any().optional(),
});

// ২. মেইন কিউ এর জন্য যখন DLX লাগবে (REQUIRED)
const retryAssertQueueOptionsForMainSchema = z
    .object({
        deadLetterExchange: z
            .string()
            .trim()
            .min(1, 'deadLetterExchange is required for DLX processes'),
        deadLetterRoutingKey: z
            .string()
            .trim()
            .min(1, 'deadLetterRoutingKey is required for DLX processes'),
    })
    .passthrough()
;

const retryAssertQueueOptionsForRetrySchema = z
    .object({
        messageTtl: z
            .number()
            .positive('messageTtl is required for retry queue'),
        deadLetterExchange: z
            .string()
            .trim()
            .min(1, 'deadLetterExchange name required for retry'),
        deadLetterRoutingKey: z
            .string()
            .trim()
            .min(1, 'deadLetterRoutingKey required for retry'),
    })
    .passthrough()
;

export const QueueConfigItemSchema = z
    .object({
        id: z.string().min(1),
        /**
         * NORMAL:          queue -> delete
         * NORMAL_DLX:      main queue -> DLX queue
         * RETRY:           main queue -> retry queue -> main queue
         * RETRY_DLX:       main queue -> retry queue: retry(x times) -> DLX queue
         */
        processType: z.enum(['NORMAL', 'NORMAL_DLX', 'RETRY', 'RETRY_DLX']),
        queueName: z.string().trim().min(1, "queue name is required"),
        queueOptions: amqpOptionsSchema,
        ...
        bindings: z
            .array(QueueBindingSchema)
            .min(1, 'At least one binding is required'),
    })
    .superRefine((data, ctx) => {

        /** NORMAL_DLX and RETRY_DLX both case DLX queue must be required */
        if (
            data.processType === 'NORMAL_DLX' ||
            data.processType === 'RETRY_DLX'
        ) {
            const mainOptionsCheck =
                retryAssertQueueOptionsForMainSchema.safeParse(
                    data.queueOptions,
                );
        ...
        /** RETRY or RETRY_DLX both case RETRY QUEUE must be required */
        ...
        if (data.retry && data.retry.retry_queueOptions) {
            ...
            const hasValidReturnRoute = data.bindings.some(
                (b) => b.exchangeName === targetDLX,
            );
        ...
        if (data.DLQ && data.queueOptions) {
            const mainDlxExchange = (data.queueOptions as any)
                .deadLetterExchange;
            const mainDlxRoutingKey = (data.queueOptions as any)
        ...
        if (data.retry && data.bindings.length > 0) {
            const firstBinding = data.bindings[0]!; //TODO: change ! it
...
export interface QueueConfigItem<T extends ProcessType> {
    id: string;
    processType: T;
    queueName: string;
    ...
    ;
    /**
     * -> Same queue binding with more then 1 exchange.
     * -> So every binding key store into a array.
     * -> So that we connect the all exchange with queue.
     */
