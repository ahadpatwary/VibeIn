...
export class StoreService {
   ...
   async reset(identifier: string, keyspace: string | null = null): Promise<void> {
      const pattern = keyspace ? `rl:*:${keyspace}:${identifier}:*` : `rl:*:${identifier}:*`;
    ...
    */
   async getStatus(identifier: string, keyspace: string | null = null): Promise<ViolationStatus> {
      const namespacedId = keyspace ? `${keyspace}:${identifier}` : identifier;

      const { results, errors } = await this.redisService.pipeline((pipe) => {
         pipe.get(`${PENALTY_KEY_PREFIX}:violations:${namespacedId}`);

         pipe.ttl(`${PENALTY_KEY_PREFIX}:ban:${namespacedId}`);
      ...
      const whitelisted = whitelistInSet === 1 || whitelistWithTTL === 1;

      const blacklisted = blacklistInSet === 1 || blacklistWithTTL === 1;
