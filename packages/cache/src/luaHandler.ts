...
export class LuaHandler {
    ...
    */
   async initialize(configs: ScriptLoaderConfig[]): Promise<void> {
   ...
   async execute<T = unknown>(name: string, keys: string[] = [], args: string[] = []): Promise<T> {
      ...
      try {
         ...
         if (this.#isNoScriptError(error)) {
            return this.#handleNoScript<T>(name, keys, args);
         }
         ...
         /** UNKNOWN error */
         throw new RedisCommandException('LUA', this.#toError(error), {
            name,
            keys,
            args,
         });
    ...
    */
   async #handleNoScript<T>(name: string, keys: string[], args: string[]): Promise<T> {
      const script = this._scripts.get(name);
      ...
      try {
          ...
          */
         const result = (await redis.eval(script.source, keys.length, ...keys, ...args)) as T;
   ...
   async #loadScript(config: ScriptLoaderConfig): Promise<LoadedLuaScript> {
      /*
    ...
    * @returns
    */
   async #reloadScript(name: string, source: string): Promise<string> {
      /*
    ...
    * @param error
    */
   #isNoScriptError(error: unknown): boolean {
