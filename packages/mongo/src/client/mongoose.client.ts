...
export class MongooseClient {
      ...
      return await withRetry(
        ...
        },
        {
          ...
          onRetry: (attempt, error, delayMs) => {
            this.logger.warn(`MongoDB connection attempt ${attempt} failed, retrying in ${delayMs}ms`, {
  ...
  private registerEventListeners(conn: Connection) {
    conn.on(MONGOOSE_CONNECTION_EVENTS.DISCONNECTED, () => this.logger.warn('MongoDB disconnected'));
    conn.on(MONGOOSE_CONNECTION_EVENTS.RECONNECTED, () => this.logger.info('MongoDB reconnected'));
