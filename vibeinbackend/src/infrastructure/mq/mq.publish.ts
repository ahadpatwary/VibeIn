...
import { PublishOptions } from './types/mq.types';
import { MessageSerializer } from './utils/mq.serializer';
import { v4 as uuidv4 } from 'uuid';
import { RabbitMQPublishException } from './exceptions/mq.exceptions';
...
export class PublishService {
    ...
    async publish<T>(  
        ...
        await this.publishWithConfirm(ch, exchangeName, routingKey, buffer, amqpOpts);
    ...
    private async publishWithConfirm(
        ...
        return new Promise<void>((resolve, reject) => {
            const ok = ch.publish(exchange, routingKey, content, opts,
                (err) => {
                    return err ? reject(
                        new RabbitMQPublishException(exchange, routingKey, err as Error)
                    ) : resolve();
                }
            );
