...
export class ChannelRecoveryModule {
    ...
    constructor(
        private readonly connection: amqp.RecoveringChannelModel,
    ){}
    ...
    private async createConfirmChannel(options: channelOptions): Promise<ConfirmChannel>{
        try {
            const ch = await this.connection.createConfirmChannel();
            await ch.prefetch(options.prefetch ?? 1);
            return ch;
        } catch (error) {
            throw new RabbitMQChannelCreationException('default', error as Error, {});
        }
    }

    private registerEventHandlers(channel: amqp.ConfirmChannel, channelName: string ): void {
