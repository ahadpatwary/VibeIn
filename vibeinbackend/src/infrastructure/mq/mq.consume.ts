...
export abstract class BaseConsumer {
    ...
    constructor(
        ...
        const rawConfig = QueueRegistry.find(item => item.id === this.serviceId);
        if (!rawConfig) {
            throw new Error(`[Consumer Error] Configuration not found for service ID: ${this.serviceId}`);
        }
    ...
    public async start(): Promise<void> {
        ...
        await this.consumer(this.config.queueName, this.handleMessageProcessing);
    }
    ...
    private async handleMessageProcessing(msg: amqp.ConsumeMessage | null): Promise<void> {
        ...
        try {
            ...
            console.log(`Message processed successfully for ID: ${this.config.id}`);
            ...
            if (msg.properties.replyTo) {
                ...
                console.log(` └─ Found replyTo. Routing response to queue: [${replyQueue}]`);
            ...
            console.error(`Error parsing message in [${this.config.id}]:`, error.message);
    ...
    private async handleFailureFlow(msg: amqp.ConsumeMessage, error: Error): Promise<void> {
        ...
        if (isRetryable && this.config.retry) {
            ...
            console.log(`🔄 Message is retryable. Count: [${currentRetryCount + 1}/${maxRetryAllowed}]. Pushing to Retry Chain...`);
            ...
            console.log(`⚠️ Max retries reached or process not retryable. Evaluating Dead-Letter/Drop Policy...`);

            if (this.config.DLQ === null) {
        
                console.log(` DLQ is null for [${this.config.id}]. Rejecting message via NACK (Requeue: false) -> Message DROPPED.`);
                this.channel.nack(msg, false, false); 
            } else {
                
                console.log(`💀 DLQ exists. Escalating to DLX Exchange: [${this.config.DLQ.dlx_exchange}] via NACK.`);
                this.channel.nack(msg, false, false);
            }
        }
    }
