...
import { RabbitMQBindingException, RabbitMQExchangeAssertException, RabbitMQQueueAssertException } from "./exceptions/mq.exceptions";
...
export class Channel {
    ...
    async setupDeadLetterTopology(
        ...
        dlxQueue: string,
        dlxQueueOptions: Options.AssertQueue & Partial<{ maxLengthBytes: string, lazy: boolean}>,
        ...
        await this.assertExchange(dlxExchange, 'direct', dlxAssertExchangeOptions);

        await this.assertQueue(dlxQueue, dlxQueueOptions);

        await this.bindQueue(dlxQueue, dlxExchange, dlxBindingKey, dlxBindQueueArg);



        await this.assertExchange(mainExchange, 'direct', mainAssertExchangeOptions);
