...
import { QueueRegistrySchema } from './your-zod-schema-file'; // তোমার জোড ফাইলের পাথ
import { QueueRegistry } from './your-config-file';        // তোমার কনফিগারেশন ফাইলের পাথ

/**
 * RabbitMQ টপোলজি ইনিশিয়ালাইজ করার মেইন ফাংশন
 * @param connection amqplib থেকে আসা সচল কানেকশন অবজেক্ট
 */
export async function initializeRabbitMQ(connection: amqp.Connection): Promise<void> {
    ...
    try {
        ...
        for (const config of validatedRegistry) {
            ...
            // ==========================================
            if (config.processType === 'NORMAL_DLX' || config.processType === 'RETRY_DLX') {
                if (config.DLQ) {
                    console.log(`  └─ 💀 Creating DLQ Exchange: ${config.DLQ.dlx_exchange}`);
                    ...
                    console.log(`  └─ 💀 Creating DLQ Queue: ${config.DLQ.dlx_queue}`);
                    await channel.assertQueue(
                        config.DLQ.dlx_queue, 
                        config.DLQ.dlx_queueOptions || { durable: true }
                    );

                    console.log(`  └─ 💀 Binding DLQ Queue to DLQ Exchange with key: ${config.DLQ.dlx_routingKey}`);
            ...
            // ==========================================
            if (config.processType === 'RETRY' || config.processType === 'RETRY_DLX') {
                if (config.retry) {
                    console.log(`  └─ 🔄 Creating Retry Exchange: ${config.retry.retry_exchange}`);
                    await channel.assertExchange(
                    ...
                    console.log(`  └─ 🔄 Creating Retry Queue: ${config.retry.retry_queue}`);
                    await channel.assertQueue(
                        config.retry.retry_queue,
                        config.retry.retry_queueOptions // এতে messageTtl ও DLX ম্যাপ করা আছে
                    );

                    console.log(`  └─ 🔄 Binding Retry Queue to Retry Exchange with key: ${config.retry.retry_routingKey}`);
                    await channel.bindQueue(
                        config.retry.retry_queue,
                        config.retry.retry_exchange,
            ...
            // ==========================================
            console.log(`  └─ 🔗 Processing ${config.bindings.length} External Exchange Bindings...`);
            for (const binding of config.bindings) {
                ...
                console.log(`     ├─ Binding Main Queue to [${binding.exchangeName}] using RoutingKey: [${binding.routingKey}]`);
                await channel.bindQueue(
                    config.queueName,
                    binding.exchangeName,
                    binding.routingKey,
