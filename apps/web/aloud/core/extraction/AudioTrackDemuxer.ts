...
export class AudioTrackDemuxer extends TypedEventEmitter<DemuxEvents> {
   ...
   async run(signal?: AbortSignal): Promise<DemuxResult> {
      ...
      if (!(this.file instanceof File)) {
         throw new DemuxError('Audio extraction requires a browser File object.');
      }

      if (this.file.size <= 0) {
         throw new DemuxError('The input file is empty.');
      }
      ...
      try {
         ...
         //
         const source = new BlobSource(this.file, {
         ...
         if (!canRead) {
            throw new DemuxError('Mediabunny could not read this media file.');
         }
         ...
         const [codec, codecParameterString, sampleRate, channelCount, duration, decoderConfig] =
            await Promise.all([
               audioTrack.getCodec(),
         ...
         if (!Number.isFinite(sampleRate) || sampleRate <= 0) {
            throw new DemuxError(`Invalid audio sample rate for track ${audioTrack.id}.`);
         }

         if (!Number.isInteger(channelCount) || channelCount <= 0) {
            throw new DemuxError(`Invalid audio channel count for track ${audioTrack.id}.`);
         }
         ...
         for await (const packet of packetSink.packets()) {
            ...
            if (packetCount === 1 || packetCount % PROGRESS_PACKET_INTERVAL === 0) {
               const percent =
                  totalDuration !== null
                     ? Math.max(
                          0,
                          Math.min(
                             99,
                             Math.floor(
                                ((packet.timestamp + packet.duration) / totalDuration) * 100,
                             ),
                          ),
                       )
                     : 0;
               ...
               if (percent !== lastProgressPercent) {
                  ...
                  this.emit('progress', {
                     bytesProcessed: Math.min(
                        this.file.size,
                        Math.floor((percent / 100) * this.file.size),
                     ),
                     percent,
         ...
         if (packetCount === 0) {
            throw new DemuxError(`No encoded audio packets were found in track ${audioTrack.id}.`);
         }
         ...
         if (!outputBuffer || outputBuffer.byteLength === 0) {
            throw new DemuxError('Mediabunny produced an empty audio output.');
         }
         ...
         const normalizedDuration =
            duration !== null && Number.isFinite(duration) && duration >= 0 ? duration : 0;
         ...
         const track: AudioTrackInfo = {
            ...
            mimeType: this.#buildMimeType(codecParameterString ?? codec),
         };
         ...
         if (error instanceof DOMException && error.name === 'AbortError') {
            throw new DemuxError('Audio demuxing was aborted.', error);
         }

         if (error instanceof Error) {
            throw new DemuxError('Unexpected error while extracting audio.', error);
         }
         ...
         try {
            ...
            console.warn('Failed to dispose Mediabunny input.', disposeError);
         }
      }
   }
