    ...
    <section id="analysis" className="border-t border-border py-24">
      <Container>
        ...
        <p className="mt-4 max-w-[58ch] text-[15.5px] leading-relaxed text-muted">
          An English mistake and a technical mistake are never the same
          thing. Aloud keeps them separate, so you always know what to fix
          and why.
        </p>
        ...
        <div className="mt-14 grid gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">
          {metrics.map((m) => (
            <div key={m.title} className="bg-surface p-7">
              ...
              <h3 className="mt-4 font-display text-[16.5px] font-semibold">
                {m.title}
              </h3>
          ...
          <div className="flex flex-col justify-center bg-surface-2 p-7">
            ...
            <p className="mt-2 text-[14px] leading-relaxed">
              Ends with the top three changes to make next time — not a wall
              of every possible correction.
            </p>
          </div>
        </div>
      </Container>
